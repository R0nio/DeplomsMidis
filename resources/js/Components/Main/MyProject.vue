...
<template>
    ...
    >
        <!-- Статус проекта (угол карточки) -->
        <div class="absolute top-3 left-3 z-10">
            <div 
                class="font-normal text-sm px-2 py-1 rounded-full"
        ...
        >
            ...
            <div v-if="project.status !== 'Отклонен' && project.status !== 'Заблокирован'" class="mt-2">
                <button
                    @click="editProject"
                    class="w-full text-sm py-2 rounded-xl transition-all duration-200 font-medium hover:opacity-90"
                    :style="{ backgroundColor: colors.accent, color: colors.bgDark, border: `1px solid ${colors.border}` }"
