...
onMounted(() => {
    ...
    if (props.project.forecasts && props.project.forecasts.length > 0) {
        revenues.value = props.project.forecasts.map(forecast => ({
            year: forecast.year,
            amount: forecast.expected_revenue
        }));
        yearlyExpenses.value = props.project.forecasts.map(forecast => ({
            year: forecast.year,
            amount: forecast.expected_expenses
        }));
    }
});
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <TitlePage id="editProject" value="Редактирование проекта"></TitlePage>
        </template>
        ...
        <div class="mx-auto py-6 px-4 sm:px-10 lg:px-16">
            <form @submit.prevent="submit" class="mx-auto">
                ...
                <div class="rounded-2xl p-6 mb-6 shadow-xl" :style="{ border: `3px solid ${colors.border}`, backgroundColor: colors.bgLight }">
                    ...
                    <!-- СТРУКТУРА ТРАТ -->
                    <div class="mt-8 pt-6" :style="{ borderTop: `2px solid ${colors.border}` }">
                        <InputLabel value="Структура трат (детализация)" :style="{ color: colors.accent }" class="text-lg mb-4" />
