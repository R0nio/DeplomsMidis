...
<template>
    <Head title="Редактирование проекта — InvestProject" />
                     ...
                     <div>
                        ...
                        <div v-if="existingPhotos.length > 0" class="mt-3">
                            ...
                            <div class="flex flex-wrap gap-3">
                                ...
                                >
                                    <img :src="photo.url" :alt="`Фото проекта ${index + 1}`" class="w-24 h-24 object-cover rounded-lg" />
                                    <button
                                        type="button"
                                        @click="removeExistingPhoto(index)"
                                        class="absolute -top-2 -right-2 bg-red-700 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs hover:bg-red-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-1 focus-visible:ring-red-700"
                        ...
                        <div v-if="photoPreviews.length > 0" class="mt-3">
                            ...
                            <div class="flex flex-wrap gap-3">
                                <div 
                                    ...
                                    class="relative w-24 h-24"
                                >
                                    <img :src="preview.url" :alt="`Новое фото ${index + 1}: ${preview.name}`" class="w-24 h-24 object-cover rounded-lg" />
                                    <button
                                        type="button"
                                        @click="removePhoto(index)"
                                        class="absolute -top-2 -right-2 bg-red-700 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs hover:bg-red-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-1 focus-visible:ring-red-700"
                                        :aria-label="`Убрать новое фото ${index + 1}`"
                                    ><span aria-hidden="true">✕</span></button>
                                </div>
                            </div>
                        </div> 
