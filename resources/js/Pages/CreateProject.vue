...
// ========== КАТЕГОРИИ ==========
const addCategory = () => {
    if (categories.value.length < 4) categories.value.push('');
};
const removeCategory = (index) => {
    if (categories.value.length > 1) categories.value.splice(index, 1);
};

// ========== СТРУКТУРА ТРАТ ==========
const addExpense = () => expenses.value.push({ item_name: '', amount: '' });
const removeExpense = (index) => {
    if (expenses.value.length > 1) expenses.value.splice(index, 1);
};
            ...
            <form @submit.prevent="submit" class="mx-auto max-w-7xl">
                ...
                <div class="rounded-2xl p-6 mb-6 shadow-lg border-2" :style="{ backgroundColor: colors.light, borderColor: colors.accent, boxShadow: shadows.lg }">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <div>
                            <InputLabel for="ownShip" value="Собственность" class="mb-2" :style="{ color: colors.brand }" />
                            <TextInput id="ownShip" type="text" class="block w-full rounded-lg" v-model="form.ownShip" placeholder="Тип собственности" />
                            <InputError class="mt-2" :message="form.errors.ownShip" role="alert" />
                        </div>
                        <div>
                            <InputLabel for="activity" value="Деятельность" class="mb-2" :style="{ color: colors.brand }" />
                            <TextInput id="activity" type="text" class="block w-full rounded-lg" v-model="form.activity" placeholder="Вид деятельности" />
                            <InputError class="mt-2" :message="form.errors.activity" role="alert" />
                        </div>
                        <div>
                            <InputLabel for="type_building" value="Вид строительства" class="mb-2" :style="{ color: colors.brand }" />
                            <TextInput id="type_building" type="text" class="block w-full rounded-lg" v-model="form.type_building" placeholder="Тип строительства" />
                            <InputError class="mt-2" :message="form.errors.type_building" role="alert" />
                        </div>
                        <div class="md:col-span-2 lg:col-span-3">
                            <InputLabel for="addres" value="Адрес *" class="mb-2" :style="{ color: colors.brand }" />
                            <TextInput id="addres" type="text" class="block w-full rounded-lg" v-model="form.addres" placeholder="Адрес проекта" />
                            <InputError class="mt-2" :message="form.errors.addres" role="alert" />
                        </div>
                        <div>
                            <InputLabel for="latitude" value="Широта *" class="mb-2" :style="{ color: colors.brand }" />
                            <TextInput id="latitude" type="text" class="block w-full rounded-lg" v-model="form.latitude" placeholder="55.7558" />
                            <InputError class="mt-2" :message="form.errors.latitude" role="alert" />
                        </div>
                        <div>
                            <InputLabel for="longitude" value="Долгота *" class="mb-2" :style="{ color: colors.brand }" />
                            <TextInput id="longitude" type="text" class="block w-full rounded-lg" v-model="form.longitude" placeholder="37.6173" />
                            <InputError class="mt-2" :message="form.errors.longitude" role="alert" />
                        </div>
                        <InputError class="mt-2" :message="form.errors.coordinates" role="alert" ></InputError>
                ...
                <div class="rounded-2xl p-6 mb-6 shadow-lg border-2" :style="{ backgroundColor: colors.light, borderColor: colors.accent, boxShadow: shadows.lg }">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                        <div>
                            <InputLabel for="total_investment" value="Требуемая сумма (₽) *" class="mb-2" :style="{ color: colors.brand }" />
                            <TextInput id="total_investment" type="number" class="block w-full rounded-lg" v-model="form.total_investment" placeholder="1000000" />
                            <InputError class="mt-2" :message="form.errors.total_investment" role="alert" />
                        </div>
                        <div>
                            <InputLabel for="collected_total_investment" value="Уже собрано (₽)" class="mb-2" :style="{ color: colors.brand }" />
                            <TextInput id="collected_total_investment" type="number" class="block w-full rounded-lg" v-model="form.collected_total_investment" placeholder="500000" />
                        ...
                        <div>
                            <InputLabel for="count_new_job" value="Рабочих мест *" class="mb-2" :style="{ color: colors.brand }" />
                            <TextInput id="count_new_job" type="number" class="block w-full rounded-lg" v-model="form.count_new_job" placeholder="10" />
                            <InputError class="mt-2" :message="form.errors.count_new_job" role="alert" />
                        </div>
                    </div>
                            ...
                            <div v-for="(revenue, index) in revenues" :key="`revenue-${index}`" class="rounded-lg p-4 transition-all border-2" :style="{ backgroundColor: colors.brand, borderColor: colors.success }">
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <div class="flex gap-2">
                                        <button v-if="revenues.length > 1" type="button" @click="removeRevenue(index)" class="p-2 rounded-lg transition-all hover:scale-110" :style="{ backgroundColor: colors.error }" aria-label="Удалить год">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" :style="{ color: colors.white }"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
                                        </button>
                                        <button v-if="index === revenues.length - 1" type="button" @click="addRevenue" class="p-2 rounded-lg transition-all hover:scale-110" :style="{ backgroundColor: colors.success }" aria-label="Добавить год">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" :style="{ color: colors.white }"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
                                        </button>
                                    </div>
                                </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                            <div v-for="(expense, index) in yearlyExpenses" :key="`yearly-expense-${index}`" class="rounded-lg p-4 transition-all border-2" :style="{ backgroundColor: colors.brand, borderColor: colors.error }">
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <div class="flex gap-2">
                                        <button v-if="yearlyExpenses.length > 1" type="button" @click="removeYearlyExpense(index)" class="p-2 rounded-lg transition-all hover:scale-110" :style="{ backgroundColor: colors.error }" aria-label="Удалить год">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" :style="{ color: colors.white }"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
                                        </button>
                                        <button v-if="index === yearlyExpenses.length - 1" type="button" @click="addYearlyExpense" class="p-2 rounded-lg transition-all hover:scale-110" :style="{ backgroundColor: colors.success }" aria-label="Добавить год">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" :style="{ color: colors.white }"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
                                        </button>
                                    </div>
                                </div>
                    ...
                    <div class="mt-8 pt-6 border-t-2" :style="{ borderColor: colors.accent }">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            <div v-for="(expense, index) in expenses" :key="`expense-${index}`" class="rounded-lg p-4 border-2" :style="{ backgroundColor: colors.brand, borderColor: colors.accent }">
                                <div class="space-y-3">
                                    <div>
                                        <InputLabel :for="`item_name-${index}`" :value="`Статья ${index + 1}`" class="mb-2" :style="{ color: colors.white }" />
                                        <TextInput :id="`item_name-${index}`" type="text" class="block w-full rounded-lg" v-model="expenses[index].item_name" placeholder="Название статьи расходов" />
                                    </div>
                                    <div>
                                        <InputLabel :for="`amount-${index}`" value="Сумма (₽)" class="mb-2" :style="{ color: colors.white }" />
                                        <TextInput :id="`amount-${index}`" type="number" step="0.01" class="block w-full rounded-lg" v-model.number="expenses[index].amount" placeholder="100000" />
                                    </div>
                                    <div class="flex gap-2 justify-end">
                                        <button v-if="expenses.length > 1" type="button" @click="removeExpense(index)" class="px-4 py-2 rounded-lg transition-all hover:scale-110" :style="{ backgroundColor: colors.error, color: colors.white }" aria-label="Удалить статью">Удалить</button>
                                        <button v-if="index === expenses.length - 1" type="button" @click="addExpense" class="px-4 py-2 rounded-lg transition-all hover:scale-110" :style="{ backgroundColor: colors.success, color: colors.white }" aria-label="Добавить статью">Добавить</button>
                                    </div>
                                </div>
                            </div>
                        </div>
